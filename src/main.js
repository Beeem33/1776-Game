...
class Game {
    ...
    document.addEventListener('pointerlockchange', () => {
      ...
      if (locked) {
        ...
        if (this.state === 'menu' || this.state === 'paused') {
          if (this.waves.state === 'idle') {
            this._startIntro(); // fresh run: cinematic opening
          } else {
            this.state = 'playing';
  ...
  // Sound runs only while actively playing (or in the opening cinematic)
  // with the game on screen
  _syncAudio() {
    const active =
      (this.state === 'playing' || this.state === 'intro') && !document.hidden;
  ...
  _firePlayerCannon(CN) {
    ...
    // The shell lands ON the crosshair: march the camera's center ray out
    // to where it meets the ground, then ballistically solve the arc to
    // exactly that spot — point, shoot, boom. No guesswork.
    const dir = new THREE.Vector3();
    this.camera.getWorldDirection(dir);
    const probe = this.camera.position.clone();
    let target = null;
    for (let i = 0; i < 150; i++) {
      probe.addScaledVector(dir, 2);
    ...
    if (!target) {
      target = this.camera.position.clone().addScaledVector(dir, 220);
      target.y = terrainHeight(target.x, target.z);
    }

    const g = 26;
    const dist = from.distanceTo(target);
    const T = THREE.MathUtils.clamp(dist / 55, 0.4, 1.9); // fast, flat arc
    const vel = target.clone().sub(from).divideScalar(T);
    vel.y = (target.y - from.y + 0.5 * g * T * T) / T;
  ...
  // C detaches you from the patriot: fly anywhere with WASD/mouse (Space
  // up, Ctrl down, Shift fast) while a competent AI keeps him fighting —
  // riding, strafing, burst-firing, reloading. C again takes command back.
  ...
  _exitFreeCam() {
    ...
    this.ui.showBanner('BACK IN COMMAND', 0.9);
  }
  ...
  _updateFreeCam(dt) {
    ...
    FC.yaw -= md.x * 0.0023;
    FC.pitch = THREE.MathUtils.clamp(FC.pitch + md.y * 0.0023, -1.5, 1.5);
    const cp = Math.cos(FC.pitch);
    const dir = new THREE.Vector3(-Math.sin(FC.yaw) * cp, -Math.sin(FC.pitch), -Math.cos(FC.yaw) * cp);
    const right = new THREE.Vector3(Math.cos(FC.yaw), 0, -Math.sin(FC.yaw));
    const fast = this.input.isDown('ShiftLeft') || this.input.isDown('ShiftRight');
    const move = new THREE.Vector3();
    if (this.input.isDown('KeyW')) move.add(dir);
    if (this.input.isDown('KeyS')) move.sub(dir);
    if (this.input.isDown('KeyD')) move.add(right);
    if (this.input.isDown('KeyA')) move.sub(right);
    if (this.input.isDown('Space')) move.y += 1;
    if (this.input.isDown('ControlLeft')) move.y -= 1;
    if (move.lengthSq() > 0) FC.pos.addScaledVector(move.normalize(), (fast ? 42 : 16) * dt);
    FC.pos.y = Math.max(FC.pos.y, terrainHeight(FC.pos.x, FC.pos.z) + 0.5);
    this.camera.position.copy(FC.pos);
    this.camera.lookAt(FC.pos.clone().add(dir));
  ...
  _updateAiPatriot(dt) {
    ...
    // Nearest living redcoat is the mark
    let target = null;
    let best = Infinity;
    for (const e of this.waves.enemies) {
      if (e.dead || e.dying || e.inFinisher) continue;
    ...
    if (target) {
      ...
      to.normalize();
      ai.strafeT -= dt;
      if (ai.strafeT <= 0) { ai.strafeT = 3 + Math.random() * 3; ai.strafeDir *= -1; }
      const side = new THREE.Vector3(-to.z, 0, to.x).multiplyScalar(ai.strafeDir);
    ...
    // the normal physics, animation and audio behavior falls out for free
    const aimYaw = target
      ? Math.atan2(target.group.position.x - P.position.x, target.group.position.z - P.position.z)
      : P.heading;
    const fakeCam = {
      yaw: aimYaw - Math.PI, // rider's upper body tracks the mark
      getGroundForward: () => (moveDir ? moveDir.clone() : new THREE.Vector3(0, 0, 1)),
      getGroundRight: () => new THREE.Vector3(),
    };
    const keys = new Set();
    if (moveDir) keys.add('KeyW');
    if (target && best > 55) keys.add('ShiftLeft'); // sprint to close distance
    P.aiming = false;
    P.update(dt, { isDown: (c) => keys.has(c) }, fakeCam);

    // Trigger discipline: bursts at anything inside 65 m, honest reloads
    if (ai.reloadT != null) {
      ai.reloadT += dt / 1.9;
      P.setReloadT(Math.min(ai.reloadT, 1));
      if (ai.reloadT >= 1) {
        ai.reloadT = null;
        ai.ammo = 25;
        P.setReloadT(null);
    ...
    if (inBurst && ai.fireT <= 0) {
      ...
      ai.ammo -= 1;
      P.kick();
      this.audio.playSfx('uzi', { volume: 0.4, rateJitter: 0.07 });
      const from = new THREE.Vector3();
      ...
      if (ai.ammo <= 0) {
        ai.reloadT = 0;
        this.audio.playSfx('reload', { volume: 0.7 });
      }
    }
  }

  // ---------------- the opening cinematic ----------------
  // Two seconds of black, then ~7 letterboxed seconds: the patriot walking
  // slowly through the wet grass toward his waiting horse, calmly seating
  // a fresh magazine — low tracking shots — before the reins are yours.
  // Space or the trigger skips it.

  _startIntro() {
    this.state = 'intro';
    this.ui.showScreen('game');
    this.ui.hud.classList.add('hidden'); // no HUD during the movie
    this._cineFade.style.transition = 'none';
    this._cineFade.style.opacity = '1';
    void this._cineFade.offsetHeight; // commit the instant blackout
    this._cineFade.style.transition = 'opacity 0.9s ease';
    this._cineTop.classList.add('in');
  ...
  _updateIntro(dt) {
    ...
    // Space or the trigger skips, once the black has lifted
    if (t > 2.4 && (this.input.isDown('Space') || this.input.fireHeld)) {
      this._endIntro();
      return;
    }
    ...
    if (t >= 2) {
      ...
      P.group.position.y = terrainHeight(P.group.position.x, P.group.position.z);
      P._footPhase += dt * 3.6;
      const swing = Math.sin(P._footPhase) * 0.5;
    ...
    if (!E.dying) {
      ...
      if (t < 1.2) {
        ...
        if (i < F.stabs) {
          ...
          P.armR.rotation.z = kf(ph, [[0, 0], [0.4, 0.35], [0.6, 0.3], [1, 0]]);
          P.armR.rotation.y = kf(ph, [[0, 0], [0.4, -0.25], [1, 0]]);
          // The whole arm drives across and down so the blade buries in
          // the MIDDLE of his chest, not the air beside his shoulder
          P.armR.position.x = kf(ph, [[0, 0.4], [0.4, 0.05], [0.6, 0.05], [1, 0.4]]);
          P.armR.position.y = kf(ph, [[0, 0.82], [0.4, 0.58], [0.6, 0.58], [1, 0.82]]);
          P.armR.position.z = kf(ph, [[0, 0.05], [0.4, 0.34], [0.6, 0.3], [1, 0.05]]);
  ...
  _endFinisher() {
    ...
    P.armR.position.set(0.4, 0.82, 0.05); // back on the shoulder
  ...
  _tick() {
    ...
    // --- opening cinematic owns the camera and the patriot ---
    if (this.state === 'intro') {
    ...
    // --- C: free camera — fly the battlefield while an AI patriot fights ---
    const cDown = this.input.isDown('KeyC');
    if (cDown && !this._cWasDown) {
      if (this.freeCam) this._exitFreeCam();
      else this._enterFreeCam();
    }
    this._cWasDown = cDown;

    if (this.freeCam) {
      this._updateFreeCam(dt);
      // Weapon idles (tracer/flash/heat fade); the AI does its own shooting
      this.weapon.update(dt, false, this.player, [], () => {}, this.camCtrl, false);
      this._updateAiPatriot(dt);
      this.audio.updateGallop(dt, this.player.speedRatio);
      ...
      for (const ev of events) {
        ...
        this.ui.flashDamage();
        this.audio.playSfx('hurt', { volume: 0.5, rateJitter: 0.1 });
      }
      this._updateShells(dt);
      ...
      this.ui.update(dt, {
        ...
        ammo: this._ai.ammo,
        magSize: 25,
        reloading: this._ai.reloadT != null,
        intermissionTime: this.waves.state === 'intermission' ? this.waves.timer : null,
      });
      this.renderer.render(this.scene, this.camera);
      return;
